...
# Circle layout (8 resources, clockwise up the production chain):
#         Sparks     (12 o'clock)
#  Uonite            Monad       (10:30 / 1:30)
# Grain               Tetrad     (9:00  / 3:00)
#  Mote              Particle    (7:30  / 4:30)
#         Iota       (6 o'clock)
...
# Volition markers
var _archon_badge_tex: Texture2D = null
...
func _preload_textures() -> void:
    ...
    if ResourceLoader.exists(badge_path):
        _archon_badge_tex = load(badge_path)
...
func _draw() -> void:
    ...
    if _icon_buttons.has(selected_resource):
        ...
        # --- Archon Volition badge: upper-right corner of each assigned icon ---
    if game_context and _archon_badge_tex:
        const BADGE_SIZE := Vector2(22.0, 22.0)
        for key in _icon_buttons:
            ...
            if vol <= 0:
                continue
            var btn   = _icon_buttons[key]
            var bpos  = btn.position + Vector2((ICON_SIZE - BADGE_SIZE.x) / 2.0, ICON_SIZE + 6.0)
            draw_texture_rect(_archon_badge_tex, Rect2(bpos, BADGE_SIZE), false)
...
func _on_plus_pressed(pool_suffix: String) -> void:
    ...
    if pool_suffix == "uonites":
        ...
        # Slot path for volitions — parent first, child fallthrough
        var free_parents: int = game_context.get_volitions_free()
        var free_children: int = game_context.get_total_free_children_in_category("allocation_wheel")
        var headroom: int = free_parents + free_children
        if headroom <= 0:
            return
        var amount: int = headroom if selected_multiplier == -1 \
                          else mini(selected_multiplier, headroom)
        game_context.begin_batch_volition_update()
        for i in amount:
            var idx = game_context.get_first_free_parent_index()
            if idx >= 0:
                game_context.assign_parent_volition(idx, "allocation_wheel", op)
            elif not game_context.assign_next_child_in_category("allocation_wheel", op):
                break
...
func _on_minus_pressed(pool_suffix: String) -> void:
    ...
    if pool_suffix == "uonites":
        ...
        if total <= 0:
            return
        var amount: int = total if selected_multiplier == -1 \
                          else mini(selected_multiplier, total)
        game_context.begin_batch_volition_update()
        for i in amount:
            if game_context.unassign_last_child_with_target("allocation_wheel", op):
                continue
            var idx = game_context.get_first_parent_index_with_target("allocation_wheel", op)
            if idx >= 0:
...
func _get_pool_locks_cost_int(_suffix: String) -> int:
    # Purity locks now consume volition slots directly; no separate cost.
    return 0
    ...
    for suffix in POOL_SUFFIXES:
        ...
        if suffix == "uonites":
            ...
            if parent_count > 0:
                ...
                row.set_label(_fmt_int(total))
            var free_parents: int = game_context.get_volitions_free()
            var free_children: int = game_context.get_total_free_children_in_category("allocation_wheel")
            if plus_btn:  plus_btn.disabled  = (free_parents + free_children <= 0)
