...
func enqueue_first_fundament(variety_key: String = "") -> void:
    ...
    var lines = [
        ...
        "Oh, sorry; I hadn't gotten to explaining that yet. Yes, we need to collapse the dimension in order to create Uonites, and then we just Expand it again. Right now, we can only store a little under a thousand material resources, but eventually we'll be able to build containers to hold more. Here, let me bring up the Stoctagon.||[reveal:stoctagon]THE WHAT NOW? OH. HEH. THAT'S ALMOST CLEVER.",
...
func enqueue_first_non_fundament(variety_key: String = "") -> void:
    ...
    var lines = [
        ...
        "Yes, and then to ten thousand of a type, which would be...divide by three, times sixty-four...ah, at least a few Expansions and Uonites away, probably. So unless you decide that we'll-meaning-mostly-[i]I'll[/i] simply grind endlessly, that'll happen a while after creating our first Uonites, which each take twenty Motes.||UNDERSTOOD. IS THERE ANY WAY TO INCREASE OUR STORAGE?",
        "Of course! Usually the Phlogiston generated by the Sparks locked up in the material resources consumed is used to create the Uonite, but it can be turned to other purposes, such as expanding our little pocket universe here. But usually that requires Persistent material resources that survive the Expansion process to make this dimension a little bigger.||AND WHAT HAPPENS TO THE OTHER SPARKS?",
