...
**Files:** `production_manager.gd` (1390), `game_data.gd` (171), `big_num.gd` (344), `resource_registry.gd` (1, dead stub — see §5).

### Shape
`sparks → monad → tetrad → particle → iota → mote → grain`, driven by per-op timers and worker counts, ticked in `production_manager.gd:_process` (193-248). `monad_compress`/`tetrad_assemble` are bespoke (random S/L/G subtype rolls, 433-478); `particle_compress`/`iota_assemble`/`mote_compress`/`grain_assemble` share a generic path (`_produce_generic` + `GENERIC_OPS` table, 59-80 & 484-518).
...
### Pain points
- ~~**Recipe costs duplicated 4x**~~ **PARTIALLY RESOLVED** (`ab45892`, `abf57e1`): `_op_has_inputs` (575-586) and `get_consumption_network` (1290-1306) now derive from `game_data.RECIPES` via a new `OP_LOCK_KEYS` const (88-95), and the single-unit manual-click functions now go through `_recipe_cost()` instead of hardcoded literals. **Still duplicated, not eliminated:** `GENERIC_OPS` (59-80) independently hardcodes the same input literals a third time — `_produce_generic` (the actual per-tick production path for particle/iota/mote/grain) reads costs from `GENERIC_OPS`, never from `RECIPES` or `_recipe_cost()`. A future cost change still has to touch two places by hand instead of four.
...
- **New this pass:** `resource_registry.gd` (see §5) is still the same 1-line dead stub (`extends Node`, no members) — the new `get_resource`/`set_resource` logic went straight into `game_context.gd` rather than into the file that reads as scaffolding for exactly this purpose.
...
**New this pass — residue from the Pitch-solver deletion:** the comment above the Sequence-axis solver (line 417) still says "...plus the pitch-axis `tone_*` equivalents further down" — those don't exist anymore, deleted on 2026-07-25 (see the deletion note now at 51-66), so this is a second copy of the exact stale claim the header docstring's fix addressed, missed because it wasn't in the header itself. Two state variables from the same deletion are also still declared and reset every `setup()` call but never populated or read by anything: `_final_pitch_clues` (179, cleared 196) and `_final_identity_clues` (172, cleared 197).
...
- **`resource_registry.gd`** — still just a 1-line dead stub (`extends Node`, no members), still referenced nowhere in the codebase, confirmed unchanged by this pass. Notably, the `get_resource`/`set_resource` generic accessor added to `game_context.gd` this refactor round (§1/§2) — functionally the exact thing this stub reads as scaffolding for — was built directly into `game_context.gd` instead, bypassing the stub entirely. That settles the "finish it" side of the old either/or: nothing is migrating into this file. The only decision left is deleting it (refactor-order item #6, updated below).
...
5. ~~Recipe costs duplicated 4x~~ **PARTIALLY DONE 2026-07-25** (branches `resource-registry-accessor`, `dedupe-manual-recipe-costs`): `_op_has_inputs`, `get_consumption_network`, and the manual single-unit functions all now derive from `game_data.RECIPES`. `GENERIC_OPS` still duplicates the same data independently — see item 8.

**Open, roughly in priority order:**
6. **Delete `resource_registry.gd`** — this pass resolved the ambiguity: nothing is migrating into it (the accessor pattern it seemed to scaffold landed directly in `game_context.gd` instead), so it's pure dead weight now, not an open design decision. Lowest-effort item on this list — a straight deletion, no logic to verify.
7. **Small cleanup from the Pitch-solver deletion** (`constellation_logic_puzzle.gd`, §3): a stale comment at ~line 417 still claims Pitch `tone_*` solver code exists "further down" (it doesn't, deleted 2026-07-25); two state vars (`_final_pitch_clues`, `_final_identity_clues`) are declared and reset every `setup()` but never populated or read by anything. Both trivial, safe, same branch of work as item 3 above — worth closing out together rather than leaving as permanent residue.
8. **`GENERIC_OPS` vs. `game_data.RECIPES`** (`production_manager.gd`, §1): the last recipe-cost duplication site. `_produce_generic` (the per-tick production path) reads costs from its own `GENERIC_OPS` table instead of `RECIPES`/`_recipe_cost()`. Mechanically similar to the just-finished manual-click dedup — same shape, smaller scope.
